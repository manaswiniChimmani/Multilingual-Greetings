import {Component} from 'react'

import Language from '../Language'

import './index.css'

const languageGreetingsList = [
  {
    id: 'bfdf40eb-eec9-4a66-a493-752fe689f0d0',
    imageUrl:
      'https://assets.ccbp.in/frontend/react-js/multilingual_greeting/english-greetings-img.png',
    buttonText: 'English',
    imageAltText: 'english',
  },
  {
    id: '0ceda891-2a0c-49e2-8c62-68e78180bac6',
    imageUrl:
      'https://assets.ccbp.in/frontend/react-js/multilingual_greeting/tamil-greetings-img.png',
    buttonText: 'Tamil',
    imageAltText: 'tamil',
  },
  {
    id: '89537778-7a46-4c58-988c-0adc931d087c',
    imageUrl:
      'https://assets.ccbp.in/frontend/react-js/multilingual_greeting/telugu-greetings-img.png',
    buttonText: 'Telugu',
    imageAltText: 'telugu',
  },
]

class Greetings extends Component {
  state = {
    activeId: languageGreetingsList[0].id,
    img: languageGreetingsList[0].imageUrl,
    altText: languageGreetingsList[0].imageAltText,
  }

  updateActiveId = (id, imageUrl, imageAltText) => {
    this.setState({activeId: id, img: imageUrl, altText: imageAltText})
  }

  render() {
    const {activeId, img, altText} = this.state

    // console.log(languageGreetingsList[activeId])

    return (
      <div className="bg-container">
        <h1 className="h1">Multilingual Greetings</h1>

        <ul className="language-buttons">
          {languageGreetingsList.map(each => (
            <Language
              key={each.id}
              details={each}
              isActive={activeId === each.id}
              updateActiveId={this.updateActiveId}
            />
          ))}
        </ul>
        <div>
          <img src={img} alt={altText} className="lan-img" />
        </div>
      </div>
    )
  }
}
export default Greetings
